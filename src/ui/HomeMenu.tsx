...
/**
 * The main menu: the name, one primary button (Play), the other three destinations as a quiet
 * list beneath it, and the field drawn from the sim's own geometry on the right. On every other screen the same four live in
    ...
    <div className="cat-cover">
      <div className="cat-cover-copy">
        ...
        <button className="cat-order" onClick={() => onNav(play.id)}>
          ...
          <span className="cat-order-h">
            Solo practice, bots, ranked and custom rooms
          </span>
          <svg
            className="cat-order-arrow"
            viewBox="0 0 24 24"
            aria-hidden="true"
          >
        ...
        <p className="cat-spec">
          Driving <b>{spec.name}</b> · {DRIVETRAIN_LABELS[spec.drivetrain]}
          {spec.teamNumber ? ` · #${spec.teamNumber}` : ""}
          {stats && (
            <>
              {" "}
              · {stats.users.toLocaleString()} players ·{" "}
              {played?.toLocaleString()} matches played
            </>
          )}
        </p>
      </div>
      ...
      <figure className="cat-plate">
        <FieldPlate />
        <figcaption>
          The {season.name} field, drawn from the same geometry the robots drive
          on.
        </figcaption>
      </figure>
